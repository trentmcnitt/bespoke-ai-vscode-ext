...
This document maps every significant AI code completion product and their features against Bespoke AI's current capabilities for gap analysis and roadmap planning.
...
| Fill-in-the-Middle (FIM) | Yes (Claude Code uses hole marker; Ollama uses native FIM) |
| Prose mode | Yes — auto-detects prose vs code by languageId |
| Multi-backend | Yes — Anthropic API, Claude Code CLI, Ollama |
...
| Context from open tabs | Partial (oracle subsystem) |
...
| Usage tracking | Yes (per-session counts + cost estimates) |
...
|---------|-------------|------------|-----------|-------------|-------------|
| **Bespoke AI** | No | No | No | No | Yes (3 backends) |
...
|---------|-----------|----------------|--------------------|--------------------|--------|
| **Bespoke AI** | Partial (oracle) | No | No | No | No |
...
|---------|------|------------|-----------|-----------|-------------|---------|
| **Bespoke AI** | Extension | Yes (Ollama) | N/A | Partial | Yes | Proprietary |
...
- **Repo:** github.com/qpd-v/Roo-Code | **Stars:** 20K+ | **License:** Apache 2.0
...
- **Multi-backend support** (Anthropic, Claude Code, Ollama) — common among open-source tools (Continue.dev supports many more)
- **Prose mode auto-detection** — no other tool has this, but the practical advantage is mild
- **Quality benchmarking with LLM-as-judge** — useful testing methodology, but closed-source products likely have similar internal QA
- **Profile system** — Cursor, Windsurf have comparable config presets
- **Post-processing pipeline** — thoughtful overlap trimming
- **Context oracle** — agent-powered file analysis for import/type/pattern awareness
...
| Feature | What's Needed | Who Has It |
|---------|--------------|-----------|
| Code actions (lightbulb fixes) | Register a CodeActionProvider, send diagnostics to LLM | Copilot, JetBrains AI |
| Rename suggestions | RenameProvider with AI-suggested names | Copilot |
| Context from open tabs | Expand context-builder to include neighboring open files | Copilot, Cursor, all major tools |
| Test generation (command) | Dedicated command that sends selection to LLM | Copilot, Tabnine, Amazon Q |
...
- **Prose mode** — auto-detecting prose vs code and tuning completion behavior per mode. No other tool does this, though the practical benefit is debatable.
- **Context oracle** — agent-powered file analysis for import/type/pattern awareness. A novel approach to context gathering, though most major tools achieve similar results through workspace indexing or open-tab analysis.
...
*Evaluated February 2026 as a potential base for Bespoke AI.*
...
| Technique | What It Does | Where to Study |
|-----------|-------------|----------------|
| **Generator reuse** | Reuse in-flight LLM stream when keystrokes match the beginning of an already-streaming completion | `core/autocomplete/generation/CompletionStreamer.ts` |
| **Stream-time filtering** | Stop generation early at suffix overlap, repetition, or function boundaries (saves tokens) | `core/autocomplete/filtering/streamTransforms/` |
| **FIM template library** | 13+ model-family templates with correct tokens/stop sequences | `core/autocomplete/templating/AutocompleteTemplate.ts` |
| **LSP context snippets** | Query go-to-definition for symbols near cursor, feed type signatures into prompt | `extensions/vscode/src/autocomplete/lsp.ts` (currently disabled) |
| **Persistent cache** | SQLite-backed LRU cache that survives editor restarts | `core/autocomplete/` |
...
| **Code actions / lightbulb** | [Roo Code](https://github.com/qpd-v/Roo-Code) | One of few open-source tools with VS Code CodeActionProvider. |
