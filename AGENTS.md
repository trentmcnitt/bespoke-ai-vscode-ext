...
Supports two backends: **Claude Code CLI** (via `@anthropic-ai/claude-agent-sdk`, requires a Claude subscription) and **direct API** (via Anthropic, OpenAI, Google Gemini, xAI, OpenRouter, or Ollama — requires an API key). The `bespokeAI.backend` setting controls which is active. Context menu commands (Explain, Fix, Do) launch an agent CLI — Claude Code (requires the Claude Code backend) or opencode (any backend, trusted workspaces only).
...
| `providers/prompt-strategy.ts` | Shared prompts: `SYSTEM_PROMPT`, `composeSystemPrompt()`, `buildFillMessage()`, `extractCompletion()` | 3 strategies: `tagExtraction` (CLI), `prefillExtraction` (Anthropic API), `instructionExtraction` (OpenAI-compat). `composeSystemPrompt()` appends the `bespokeAI.customInstructions` setting (subordinated to core rules; completions only) after `sanitizeCustomInstructions()` — control/bidi chars stripped, capped at 2000 chars |
...
| `commands/context-menu.ts`   | Explain/Fix/Do — launches Claude CLI or opencode                         | Pure helpers in `commands/context-menu-utils.ts`. Does **not** use pool server                                                                                                               |
| `mode-detector.ts`           | `languageId` → `prose` or `code`                                         | Unknown languages default to prose                                                                                                                                                           |
| `utils/claude-executable.ts` | Resolves the Claude Code executable for the SDK spawn                    | Prefers a native binary (`~/.local/bin/claude[.exe]` → PATH) so startup works without `node` on PATH; falls back to bundled `cli.js`. Memoized                                               |
...
| `utils/api-health.ts`        | Classify/describe API health-check failures (billing/auth/transient)     | Pure helpers for the background API health check (`runApiHealthCheck` in `extension.ts`), which runs when the API backend becomes active                                                     |
| `utils/cache.ts`             | LRU completion cache                                                     |                                                                                                                                                                                              |
| `utils/preset-audit.ts`      | One-time heuristic review of persisted custom presets                    | Pure. Flags `baseUrl` that is neither loopback nor a built-in provider host, non-default `apiKeyEnvVar`, `extraHeaders`. `runPresetAudit()` in `extension.ts` shows it once, Keep by default |
...
**Custom-instructions scenarios:** `scenarios/custom-instructions.ts` sets a `customInstructions` value on each `TestScenario` (the `bespokeAI.customInstructions` setting). The runner threads it into the provider config and saves it into `input.json`, and the validator judges on two axes — the steer is honored AND the completion isn't degraded / the instruction never leaks. This is the coverage for the custom-instructions feature; any change to `composeSystemPrompt()` should be re-run against these across the reference model set.
...
- **Context menu commands (Explain, Fix, Do) launch an agent CLI, not a backend.** `bespokeAI.contextMenu.agent` picks `claude-code` or `opencode` (validated on load); they do not use the pool server or the API backend. The menu is hidden via `bespokeAI.cliAvailable` unless the agent is `opencode` or the backend is `claude-code`. They do not check `bespokeAI.enabled` — they work even when inline completions are disabled.
- **opencode requires a trusted workspace and ignores `contextMenu.permissionMode`.** opencode has no trust prompt of its own and runs a repo's `opencode.json` MCP commands, `.opencode/` plugin code, and provider redirects at startup (verified), so `resolveAgentLaunch()` refuses it when `vscode.workspace.isTrusted` is false. `OPENCODE_PERMISSION` replaces the user's `opencode.json` rules per key (verified: env `bash: ask` overrode a configured `bash: deny`), so our modes are not mapped onto it; the user's opencode config governs, and its default allows edits and shell commands.
