...
| `utils/preset-audit.ts`      | One-time heuristic review of persisted custom presets                    | Pure. Flags non-loopback `baseUrl`, non-default `apiKeyEnvVar`, `extraHeaders`. `runPresetAudit()` in `extension.ts` shows it once, Keep by default |
