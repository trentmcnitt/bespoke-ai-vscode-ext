...
const CODE_LANGUAGES = new Set([
  ...
  'ruby', 'php', 'swift', 'kotlin', 'scala', 'haskell', 'lua',
  'perl', 'r', 'dart', 'elixir', 'erlang', 'clojure', 'fsharp',
  'objective-c', 'objective-cpp', 'groovy', 'powershell',
  'shellscript', 'bash', 'zsh', 'fish',
  'sql', 'graphql',
...
const PROSE_LANGUAGES = new Set([
  'markdown', 'plaintext', 'latex', 'restructuredtext', 'txt',
  'asciidoc', 'org',
]);
...
export class ModeDetector {
  detectMode(languageId: string, config: ExtensionConfig): CompletionMode {
    ...
    if (config.mode === 'prose') { return 'prose'; }
    if (config.mode === 'code') { return 'code'; }
    ...
    // Auto-detect
    if (PROSE_LANGUAGES.has(languageId)) { return 'prose'; }
    if (CODE_LANGUAGES.has(languageId)) { return 'code'; }
