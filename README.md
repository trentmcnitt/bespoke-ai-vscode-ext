...
**Claude Code** — via `@anthropic-ai/claude-agent-sdk`. The extension spawns Claude Code subprocesses and reuses them across multiple completions (2-slot rotating pool, up to 24 completions per slot before recycling). Uses a `>>>CURSOR<<<` marker approach with `<completion_start>` anchoring. Same prompt structure for prose and code — the model infers the content type.
...
**Session reuse.** Each Claude Code subprocess serves up to 24 completions before recycling. A 2-slot pool ensures one slot is always warming up while the other serves requests.
